module ActsAsMarkable
  class WrongMarkableType < Exception
    def initialize(markable_name)
      super "Wrong markable type: ['#{ActsAsMarkable.markables.join("', '")}'] expected, '#{markable_name}' provided."
    end
  end
  ...
  class WrongMarkerType < Exception
    def initialize(marker_name)
      super "Wrong marker type: ['#{ActsAsMarkable.markers.join("', '")}'] expected, '#{marker_name}' provided."
    end
  end
